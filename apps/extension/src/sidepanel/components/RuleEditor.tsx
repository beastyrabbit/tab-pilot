	...
	return (
		<div className="fixed inset-0 bg-black/30 flex items-start justify-center pt-4 z-50">
			<div className="bg-white rounded-xl shadow-xl w-[380px] max-h-[90vh] overflow-y-auto">
				...
				<div className="p-2">
					{rules.length === 0 ? (
						<p className="text-xs text-gray-400 text-center py-4">No rules yet. Add one above.</p>
					) : (
						<div className="space-y-1">
							{rules.map((rule) => (
								<div
									key={rule.id}
									className={`flex items-center gap-2 p-2 rounded text-xs ${rule.enabled ? "bg-white" : "bg-gray-100 opacity-60"}`}
								>
									<input
										type="checkbox"
									...
									<div className="flex-1 min-w-0">
										...
										<div className="text-[10px] text-gray-500">→ {rule.targetGroup}</div>
									</div>
									<button
										type="button"
